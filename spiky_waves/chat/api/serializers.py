...
class UserAccountEditSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if email:
            if instance.email != email:
                ...
                if CustomUser.objects.filter(email=email).exclude(pk=instance.pk).exists():
                ...
                raise ValidationError({'detail': 'New email is the same as the current email'})
    ...
    def to_representation(self, instance):
        ...
        try:
            ...
            EmailChangeCode.objects.create(user=instance, code=cg.generate_verification_code())
            v_code = cg.generate_verification_code()
            cg.send_verification_email(instance, v_code)
